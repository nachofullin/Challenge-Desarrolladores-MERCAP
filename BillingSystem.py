...
class Bill:
    ...
    def calculate_total(self):
        ...
        for call in self.calls:
            if call.category == 'local':
                total += self.calculate_local_cost(call)
            elif call.category == 'national':
                total += self.calculate_national_cost(call)
            elif call.category == 'international':
                total += self.calculate_international_cost(call)
        return total
    ...
    def calculate_local_cost(self, call):
        ...
        if day <= 4:    #Monday to Fridat
            if 8 <= hour < 20:
                fee = call.duration * 0.20
            else:
                fee = call.duration * 0.10
        else:       #Saturday and Sunday
            fee = call.duration * 0.10      #CCould be simplified in just one if since 0.10 is repeated but kept it the way it was stated in the instructions for posible changes in fees
        return fee
    ...
    def print_bill(self):
        ...
        for index, call in enumerate(self.calls, start=1):
            print(f'{index}. Date: {call.date_time}, Duration: {call.duration}, Category: {call.category}, Destination: {call.destination}')
        print(f'Total amount to pay: {total_payment}')

# Example
phone_bill = Bill('November', 15.00)
